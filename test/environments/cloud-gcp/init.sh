...
echo "::group::Data plane cluster-gcp initialization"
KUBECONFIG="${KUBECONFIG_DIR}/cluster-gcp.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
echo ferryctl data-plane init --tunnel-service-type=LoadBalancer
ferryctl data-plane init --tunnel-service-type=LoadBalancer
echo "::endgroup::"

echo "::group::Data plane cluster-gcp join"
KUBECONFIG="${KUBECONFIG_DIR}/control-plane.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
FERRY_PEER_KUBECONFIG="${KUBECONFIG_DIR}/cluster-azure.yaml"
echo "FERRY_PEER_KUBECONFIG=${FERRY_PEER_KUBECONFIG}"
echo ferryctl control-plane join cluster-gcp --control-plane-reachable=false
ferryctl control-plane join cluster-gcp --control-plane-reachable=false
echo "::endgroup::"

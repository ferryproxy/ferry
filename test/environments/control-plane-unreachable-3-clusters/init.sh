...
KUBECONFIG="${KUBECONFIG_DIR}/control-plane.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
echo ferryctl control-plane join cluster-1 "--data-plane-tunnel-address=${HOST_IP}:31001" --control-plane-reachable=false --data-plane-reception=control-plane
SEND_TO_CLUSTER_1="$(ferryctl control-plane join cluster-1 "--data-plane-tunnel-address=${HOST_IP}:31001" --control-plane-reachable=false --data-plane-reception=control-plane 2>/dev/null)"
echo "::endgroup::"
...
KUBECONFIG="${KUBECONFIG_DIR}/control-plane.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
echo ferryctl control-plane join cluster-2 "--data-plane-tunnel-address=${HOST_IP}:31002" --control-plane-reachable=false --data-plane-reception=control-plane
SEND_TO_CLUSTER_2="$(ferryctl control-plane join cluster-2 "--data-plane-tunnel-address=${HOST_IP}:31002" --control-plane-reachable=false --data-plane-reception=control-plane 2>/dev/null)"
echo "::endgroup::"

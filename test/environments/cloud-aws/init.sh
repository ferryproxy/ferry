...
echo "::group::Data plane cluster-aws initialization"
KUBECONFIG="${KUBECONFIG_DIR}/cluster-aws.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
echo ferryctl data-plane init --tunnel-service-type=LoadBalancer
ferryctl data-plane init --tunnel-service-type=LoadBalancer
echo "::endgroup::"

echo "::group::Data plane cluster-aws join"
KUBECONFIG="${KUBECONFIG_DIR}/control-plane.yaml"
echo "KUBECONFIG=${KUBECONFIG}"
FERRY_PEER_KUBECONFIG="${KUBECONFIG_DIR}/cluster-aws.yaml"
echo "FERRY_PEER_KUBECONFIG=${FERRY_PEER_KUBECONFIG}"
echo ferryctl control-plane join cluster-aws --control-plane-reachable=false
ferryctl control-plane join cluster-aws --control-plane-reachable=false
echo "::endgroup::"
